<h1 align="center">Hi 👋, I'm Ademola Omosanya</h1>
<h3 align="center">Software Engineer • Open Source Contributor • Building modern web experiences</h3>

<p align="center">
  I enjoy building scalable web applications, contributing to open source, and learning how browsers, rendering engines, and distributed systems work.
</p>

---

## About Me
- 💻 Software Engineer specializing in **Next.js, React, TypeScript, Node.js, PHP, NestJS, Mediawiki and Python**
- ❤️ Passionate about Open Source and Web Platform technologies
- 🌱 Currently learning **Browser Engine Architecture, DSA, System Design, Rust, and AI Engineering**
- 👨‍💻 Outreachy Intern & Wikimedia Open Source Contributor
- 🎯 Working toward becoming a world-class software engineer
- 💼 Open to Software Engineering Internships and Full-Time Opportunities

## Experience & Milestones

#### 💼 Experience

- Software Engineering Intern — Outreachy × Wikimedia Foundation
- Open Source Mentor — Wikimedia Foundation
- Open Source Contributor — MediaWiki ecosystem

#### 🌍 Milestones

- 🎓 Outreachy Intern (Wikimedia Foundation, 2025)
  - Enhanced VisualData by bringing JSON Schema support into MediaWiki.
  -  [*Phabricator*](https://phabricator.wikimedia.org/T387039/)

- 👨‍🏫 Outreachy Mentor (Wikimedia Foundation, 2026)
  - Co-mentored the Python rewrite of PendingChangesBot, helping contributors build and deploy production-ready features.
  - [*Phabricator*](https://phabricator.wikimedia.org/T405726)


- 🏅 Wikimedia Hackathon 2026 Scholar
  - Selected to attend the Wikimedia Hackathon in Milan through scholarship.
  - Closing ceremony timestaps: [My Speech](https://youtu.be/rHMTJB8gleQ?t=2160) and [Reference](https://youtu.be/rHMTJB8gleQ?t=1599)
  - [*Hackathon participant page*](https://www.mediawiki.org/wiki/Wikimedia_Hackathon_2026/Participants#Participant_List)
 

## My Current Focus

- 🚀 Building production-grade full stack applications
- 🌍 Continue contributing to open source
- 🦀 Learning Rust/C++
- 🌐 Building Projects (Clyyo, Ferrum & Miivra)
- 🤖 Exploring AI Engineering
- 📚 Deepen my understanding of distributed systems,  browser internals and AI engineering

---

## Connect With Me

- LinkedIn: [*Linkedin*](https://www.linkedin.com/in/ademola-omosanya/)
- Portfolio: [*Porfolio*](https://ademolaomosanya.github.io/experience.html)


## Fun Fact

I transitioned from Meteorology to Software Engineering through open source, and now I'm passionate about building software that impacts developers worldwide.
